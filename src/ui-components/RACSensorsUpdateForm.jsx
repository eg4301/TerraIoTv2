...
import { getRACSensors } from "../graphql/queries";
import { updateRACSensors } from "../graphql/mutations";
const client = generateClient();
export default function RACSensorsUpdateForm(props) {
  const {
    id: idProp,
    rACSensors: rACSensorsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = rACSensorsRecord
      ? { ...initialValues, ...rACSensorsRecord }
      : initialValues;
  ...
  const [rACSensorsRecord, setRACSensorsRecord] =
    React.useState(rACSensorsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getRACSensors.replaceAll("__typename", ""),
              variables: { ...idProp },
            })
          )?.data?.getRACSensors
        : rACSensorsModelProp;
      setRACSensorsRecord(record);
    };
    queryData();
  }, [idProp, rACSensorsModelProp]);
  React.useEffect(resetStateValues, [rACSensorsRecord]);
  const validations = {
    CO2: [],
    MAC: [{ type: "Required" }],
    O2: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateRACSensors.replaceAll("__typename", ""),
            variables: {
              input: {
                MAC: rACSensorsRecord.MAC,
                timestamp: rACSensorsRecord.timestamp,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "RACSensorsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Co2"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || rACSensorsModelProp)}
